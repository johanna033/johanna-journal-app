...
photoInput.addEventListener('change', async event => {
  ...
  selectedPhotos = await Promise.all(files.map(file => resizeImage(file)));
  renderPhotoPreview();
});

function resizeImage(file, maxSize = 1100, quality = 0.78) {
...
saveEntryButton.addEventListener('click', () => {
  ...
  try {
    ...
    alert('Die Fotos sind leider zu groß zum lokalen Speichern. Versuch bitte weniger oder kleinere Fotos.');
  }
});
